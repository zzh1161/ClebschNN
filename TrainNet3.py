...
f = torch.sin(pos_x) + torch.sin(pos_y)
f = f.to(device)
class Dataset(nn.Module):
    def __init__(self, data_type='train'):
        if data_type == 'train':
            psi1, psi2 = tube_vortex(pos_x, pos_y)
            ...
            psi1_n, psi2_n = schroedinger_solver.advance_with_f(f, psi1, psi2, Delta_t)
...
def train():
    n_epoch = 500
    optimizer = torch.optim.Adam(advectionNN.parameters(), lr=0.0015)
